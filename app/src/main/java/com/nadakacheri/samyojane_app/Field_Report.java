...
public class Field_Report extends AppCompatActivity {
    ...
    public void GetWardName(final int town_Code){
       /* sqlLiteOpenHelper_class = new SqlLiteOpenHelper_Class(Field_Report.this,"str","str");
        ...
        Cursor cursor3 = databaseWardName.rawQuery("select * from "+DataBaseHelperClass_WardNames.TABLE_NAME
                +" where "+DataBaseHelperClass_WardNames.TWCODE + "=" + town_Code ,null);
        if (cursor3.getCount()>0) {
            if (cursor3.moveToNext()) {
                objects_Ward.add(new AutoCompleteTextBox_Object(cursor3.getString(cursor3.getColumnIndexOrThrow((DataBaseHelperClass_TownNames.CODE))), (cursor3.getString(cursor3.getColumnIndexOrThrow((DataBaseHelperClass_TownNames.ENGLISHNAME))))));
                Log.d("serviceName", "" + objects_Ward.size());
            }*/
        objects_Ward.clear();
    ...
    public void display_Urban_Data_AfterItemSelected(int townCode, int wardCode) {
        ...
        if(cursor.getCount()>0) {
            ...
            Log.d("InDisplayIf", ""+ i);
            ur_list_adapter = new UR_List_Adapter(Field_Report.this, SlNo, Service_Name, TotalCount, TownName, TownCode, WardName, WardCode, Option_Flag);
