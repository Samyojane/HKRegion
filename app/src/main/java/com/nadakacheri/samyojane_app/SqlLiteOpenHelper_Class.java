...
//    static String ID_Ename = "ID_Ename";
    ...
    public SqlLiteOpenHelper_Class(Context context, int data){
        super(context, DATABASE_NAME_District_master, null, DATABASE_VERSION);
        ctx = context;
    }

    public SqlLiteOpenHelper_Class(Context context, int data, int data1){
        super(context, DATABASE_NAME_Taluk_master, null, DATABASE_VERSION);
        ctx = context;
    }

    public SqlLiteOpenHelper_Class(Context context, int data, int data1, int data2){
    ...
    public List<SpinnerObject> GetDocsName() {
        ...
        try {
            ...
            objects.add ( new SpinnerObject( "0" , "-- Select --") );
            objects.add(new SpinnerObject("0", "Select Document"));
            if (cursor.getCount() > 0) {
                if (cursor.moveToNext()) {
                    do {
    ...
    private void CopyHKDistrictDataBaseFromAsset() throws IOException {
        ...
        // Path to the just created empty db
        String outFileName = getDistrictMasterDatabasePath();
    ...
    private static String getDistrictMasterDatabasePath() {
    ...
    public void open_HK_District_Tbl() throws SQLException {
    ...
    public List<SpinnerObject> Get_HKDistrict(String add){
        ...
        try{
            ...
            cursor = database.rawQuery("select * from "+Table_DISTRICT_MASTER+" where "+LDM_Old_NK_DistrictCode+" = "+4+" or "
                    ...
                    +LDM_Old_NK_DistrictCode+" = "+12+" or "+LDM_Old_NK_DistrictCode+" = "+30+" or "+LDM_Old_NK_DistrictCode+" = "+31 +" order by "+LDM_district_ename,null);
            objects.add ( new SpinnerObject( "0" , add) );
            if (cursor.getCount()>0){
                if(cursor.moveToNext()) {
                    do {
                        objects.add(new SpinnerObject(cursor.getString(cursor.getColumnIndex(LDM_district_code)), cursor.getString(cursor.getColumnIndex(LDM_district_ename))));
                    } while (cursor.moveToNext());
                }
            }
            else {
    ...
    public List<SpinnerObject> Get_HKTaluk(String add, int distCode){
        ...
        try{
            ...
            if (cursor.getCount()>0){
                if(cursor.moveToNext()) {
                    do {
                        objects.add(new SpinnerObject(cursor.getString(cursor.getColumnIndex(LTM_Taluk_code)), cursor.getString(cursor.getColumnIndex(LTM_Taluk_eName))));
                    } while (cursor.moveToNext());
                }
            }
            else {
    ...
    public List<SpinnerObject> Get_HKHobli(String add, int talukCode){
        ...
        try{
            database = this.getReadableDatabase();
            if(talukCode ==0) {
                cursor = database.rawQuery("select * from " + Table_Hobli_MASTER + " order by " + LHM_Hobli_eName, null);
            }else {
                cursor = database.rawQuery("select * from " + Table_Hobli_MASTER + " where "+LHM_Taluk_code + " = "+talukCode+" order by " + LHM_Hobli_eName, null);
            }
            objects.add ( new SpinnerObject( "0" , add) );
            if (cursor.getCount()>0){
                if(cursor.moveToNext()) {
                    do {
                        objects.add(new SpinnerObject(cursor.getString(cursor.getColumnIndex(LHM_Hobli_Code)), cursor.getString(cursor.getColumnIndex(LHM_Hobli_eName))));
                    } while (cursor.moveToNext());
                }
            }
            else {
